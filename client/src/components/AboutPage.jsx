        ...
        <div className={aboutUsStyles.container}>
            <section className={aboutUsStyles.heroSection}>
                <div className={aboutUsStyles.heroBackground}>
                    <div style={{ backgroundImage: `url(${AboutBanner})`, opacity: 0.85 }} className={aboutUsStyles.heroImageContainer} />
                    <div
                    ...
                    <div className={aboutUsStyles.statsGrid}>
                        {statsMeta.slice(0, 4).map((s, i) => (
                            <div key={i} className={aboutUsStyles.statCard} style={{ minWidth: 120 }}>
                                <div className={aboutUsStyles.statNumber}>{formatStatNumber(s.key)}</div>
                                <div className={aboutUsStyles.statLabel}>{s.label}</div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {missionVisionValues.map((sec, i) => {
                const Icon = sec.icon;
                return (
                    <section key={sec.type} className={`${aboutUsStyles.sectionContainer} ${sec.bgColor} ${i % 2 === 1 ? "bg-white" : ""}`}>
                        <div className={aboutUsStyles.sectionGrid}>
                            <div className={`${aboutUsStyles.sectionContentGrid} ${i % 2 === 1 ? "lg:grid-flow-dense" : ""}`}>
                                <div className={`${aboutUsStyles.sectionImageContainer} ${i % 2 === 1 ? "col-start-2" : ""}`}>
                                    ...
                                    <div className={aboutUsStyles.featuresContainer}>
                                        {sec.features.map((f, i) => (
                                            <div key={i} className={aboutUsStyles.featureItem}>
                                                <div className={`${aboutUsStyles.featureIcon} ${sec.color}`}>
                                                    <BadgeCheck className={aboutUsStyles.featureIconSvg} />
                                                </div>
                                                <span className={aboutUsStyles.featureText}>{f}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                    ...
                    <div className={aboutUsStyles.teamGrid}>
                        {teamMembers.map((t, i) => (
                            <div key={i} className={aboutUsStyles.teamMember}>
                                ...
                                <h3 className={aboutUsStyles.teamName}>{t.name}</h3>
                                <div className={aboutUsStyles.teamRole}>{t.role}</div>
                                <p className={aboutUsStyles.teamBio}>{t.bio}</p>
                            </div>
                        ))}
                    </div>
                </div>
